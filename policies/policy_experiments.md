...
- Программа [*`thread_runner.cpp`*](./policies/thread_runner.cpp) с классом, который запускает потоки с определенными настройками для планировщика
- Программа [*`policy.cpp`*](./policies/policy.cpp), использующая `ThreadRunner` и передающая ему тесты для экспериментов
